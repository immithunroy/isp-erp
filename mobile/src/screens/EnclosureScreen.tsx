...
export function EnclosureScreen() {
  const { isOnline, triggerSync } = useSync();
  const [profile, setProfile] = useState<MobileProfile | null>(null);
  const [gps, setGps] = useState<{latitude: number; longitude: number; accuracy: number} | null>(null);  const [assetCode, setAssetCode] = useState("ENC");
  const [assetName, setAssetName] = useState("Enclosure");
  ...
  const handleSubmit = useCallback(() => {
    ...
    Alert.alert("Saved", "Enclosure recorded. It will sync automatically.", [
      { text: "OK", onPress: () => { setAssetCode("ENC"); setAssetName("Enclosure"); setStatus("active"); setCapacity(""); setNotes(""); } },
    ]);
  }, [assetCode, assetName, status, capacity, notes, gps, isOnline, triggerSync]);

  return (
    <ScrollView style={styles.container}>
      <Card title="Enclosure Capture">
        <Text style={styles.desc}>Capture an enclosure network asset.</Text>
        <GPSCapture onCapture={setGps} maxAccuracy={50} />
        {gps && (
          <View>
            <Text style={styles.label}>Asset Code *</Text>
            <TextInput style={styles.input} value={assetCode} onChangeText={setAssetCode} placeholder="ENC-001" />
            <Text style={styles.label}>Asset Name *</Text>
            <TextInput style={styles.input} value={assetName} onChangeText={setAssetName} placeholder="Enclosure Name" />
            ...
            <TouchableOpacity style={styles.btn} onPress={handleSubmit} disabled={!assetCode || !gps}>
              <Text style={styles.btnText}>Save Enclosure</Text>
            </TouchableOpacity>
          </View>
        )}
      </Card>
