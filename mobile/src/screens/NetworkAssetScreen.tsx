...
export function NetworkAssetScreen() {
  const { isOnline, triggerSync } = useSync();
  const [profile, setProfile] = useState<MobileProfile | null>(null);
  const [gps, setGps] = useState<{latitude: number; longitude: number; accuracy: number} | null>(null);  const [assetCode, setAssetCode] = useState("asset_code");
  ...
  const handleSubmit = useCallback(() => {
    ...
    Alert.alert("Saved", "name recorded. It will sync automatically.", [
      { text: "OK", onPress: () => { setAssetCode("asset_code"); setAssetName("name"); setStatus("active"); setCapacity(""); setNotes(""); } },
    ]);
  }, [assetCode, assetName, status, capacity, notes, gps, isOnline, triggerSync]);

  return (
    <ScrollView style={styles.container}>
      <Card title="Network Asset Capture">
        <Text style={styles.desc}>Capture or view a network asset at your location.</Text>
        <GPSCapture onCapture={setGps} maxAccuracy={50} />
        {gps && (
          <View>
            <Text style={styles.label}>Asset Code *</Text>
            <TextInput style={styles.input} value={assetCode} onChangeText={setAssetCode} placeholder="asset_code-001" />
            <Text style={styles.label}>Asset Name *</Text>
            <TextInput style={styles.input} value={assetName} onChangeText={setAssetName} placeholder="name Name" />
